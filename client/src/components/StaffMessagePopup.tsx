...
const StaffMessagePopup: React.FC<StaffMessagePopupProps> = ({ messages, onSend, onClose, loading }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div
        ref={modalRef}
        className="bg-white rounded-xl shadow-xl p-5 w-full max-w-md relative flex flex-col"
        role="dialog"
        aria-modal="true"
        aria-labelledby="popup-title"
        tabIndex={-1}
      >
        <button className="absolute top-2 right-2 text-gray-500 hover:text-blue-700" onClick={onClose} aria-label="Đóng popup">&times;</button>
        ...
        <div className="flex gap-2 mt-2 items-center">
          ...
          />
          <button
            className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1 rounded font-semibold flex items-center justify-center"
            onClick={handleSend}
