...
export interface IStorage {
  ...
  getAllOrders(filter: { status?: string; roomNumber?: string, limit?: number, offset?: number }): Promise<Order[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllOrders(filter: { status?: string; roomNumber?: string, limit?: number, offset?: number }): Promise<Order[]> {
    const whereClauses = [];
    if (filter.status) {
    ...
    let limitedQuery = filter.limit !== undefined ? filteredQuery.limit(filter.limit) : filteredQuery;
    let offsetQuery = filter.offset !== undefined ? limitedQuery.offset(filter.offset) : limitedQuery;
