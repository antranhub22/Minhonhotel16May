...
function logOpenAI(data: any) {
  try {
    ...
    if (safeData.response && typeof safeData.response === 'string') {
      safeData.response = safeData.response.slice(0, 1000) + (safeData.response.length > 1000 ? '...[truncated]' : '');
    }
    fs.appendFileSync(logPath, `[${new Date().toISOString()}] ${JSON.stringify(safeData)}\n`);
  } catch (err) {
    // Không throw lỗi log
  }
}
