...
var pool = new Pool({
  ...
  maxUses: 7500
  // Close a connection after it has been used this many times
});
...
var DatabaseStorage = class {
  ...
  async getAllOrders(filter) {
    const whereClauses = [];
    if (filter.status) {
      whereClauses.push(eq(orders.status, filter.status));
    }
    if (filter.roomNumber) {
      whereClauses.push(eq(orders.roomNumber, filter.roomNumber));
    }
    let baseQuery = db.select().from(orders);
    let filteredQuery = whereClauses.length > 0 ? baseQuery.where(and(...whereClauses)) : baseQuery;
    let limitedQuery = filter.limit !== void 0 ? filteredQuery.limit(filter.limit) : filteredQuery;
    let offsetQuery = filter.offset !== void 0 ? limitedQuery.offset(filter.offset) : limitedQuery;
...
function logOpenAI(data) {
  try {
    const logDir = path.join(__dirname, "logs");
    if (!fs.existsSync(logDir)) fs.mkdirSync(logDir);
    const logPath = path.join(logDir, "openai.log");
    const safeData = { ...data };
    if (safeData.prompt && typeof safeData.prompt === "string") {
      safeData.prompt = safeData.prompt.slice(0, 500) + (safeData.prompt.length > 500 ? "...[truncated]" : "");
    }
    if (safeData.response && typeof safeData.response === "string") {
      safeData.response = safeData.response.slice(0, 1e3) + (safeData.response.length > 1e3 ? "...[truncated]" : "");
    }
    fs.appendFileSync(logPath, `[${(/* @__PURE__ */ new Date()).toISOString()}] ${JSON.stringify(safeData)}
`);
  } catch (err) {
  }
}
...
var authLimiter = rateLimit({
  ...
  // 1 hour
  max: 5,
  // Limit each IP to 5 login attempts per hour
...
var emailLimiter = rateLimit({
  ...
  // 1 hour
  max: 10,
  // Limit each IP to 10 email sends per hour
...
async function healthCheck(req, res) {
  ...
  try {
    await cacheService.set("health-check", "ok", 1e3);
...
function handleApiError(res, error, message2) {
...
async function translateToVietnamese(text3) {
  try {
    ...
    return response.choices[0]?.message?.content || text3;
  } catch (error) {
...
var emailQueue = new Bull("email-queue", {
  redis: {
      ...
      try {
        ...
        if (data.type === "init" && data.callId) {
          ...
          if (!wsClients.has(data.callId)) {
            wsClients.set(data.callId, /* @__PURE__ */ new Set());
          }
...
async function registerRoutes(app2) {
  ...
  app2.get("/health", healthCheck);
  app2.get("/health/detailed", detailedHealthCheck);
  ...
  app2.patch("/api/orders/:id/status", authenticateJWT, async (req, res) => {
  ...
  app2.get("/api/staff/orders", authenticateJWT, async (req, res) => {
    try {
  ...
  app2.post("/api/orders/:id/update-status", authenticateJWT, async (req, res) => {
    const idNum = parseInt(req.params.id, 10);
    const { status } = req.body;
    try {
      const updatedOrder = await storage.updateOrderStatus(idNum, status);
  ...
  app2.get("/api/summaries/:callId", cache({ ttl: 3e5 }), async (req, res) => {
    try {
      const callId = req.params.callId;
      if (/^\d+$/.test(callId)) {
  ...
  app2.get("/api/summaries/recent/:hours", cache({ ttl: 300 }), async (req, res, next) => {
    try {
  ...
  app2.post("/api/translate-to-vietnamese", cache({ ttl: 3600 }), async (req, res, next) => {
    try {
  ...
  app2.post("/api/send-service-email", emailLimiter, async (req, res) => {
    try {
  ...
  app2.post("/api/send-call-summary-email", emailLimiter, async (req, res) => {
    try {
  ...
  app2.post("/api/mobile-test-email", emailLimiter, async (req, res) => {
    try {
...
Mi Nhon Hotel Mui Ne`
  ...
  app2.post("/api/mobile-call-summary-email", emailLimiter, async (req, res) => {
    try {
  ...
  app2.post("/api/staff/login", authLimiter, async (req, res) => {
  ...
  app2.get("/api/staff/requests", authenticateJWT, cache({ ttl: 3e4 }), async (req, res) => {
    try {
      const dbRequests = await db.select().from(request);
  ...
  app2.patch("/api/staff/requests/:id/status", authenticateJWT, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { status } = req.body;
      if (!status) {
      ...
      const result = await db.update(request).set({
        status,
        updatedAt: /* @__PURE__ */ new Date()
  ...
  app2.get("/api/staff/requests/:id/messages", authenticateJWT, (req, res) => {
    const id = parseInt(req.params.id);
    const msgs = messageList.filter((m) => m.requestId === id);
    res.json(msgs);
  });
  app2.post("/api/staff/requests/:id/message", authenticateJWT, (req, res) => {
    const id = parseInt(req.params.id);
  ...
  app2.delete("/api/staff/requests/all", authenticateJWT, async (req, res) => {
    try {
...
function serveStatic(app2) {
  ...
  app2.use("*", (req, res) => {
    ...
    if (reqPath.includes(".")) {
      const filePath = path3.join(distPath2, reqPath);
